test_that("Crane Rank Function Works", {
    testMat <- matrix(
    ...
    testRank <- craneRankFunction(testMat)
...
test_that("Score Function Works",{
    ...
    rankMat <- craneRankFunction(testMat)
    score <- craneScoreFunction(rankMat)
    expect_type(score, "double")

    identicalRank <- matrix(seq_len(7), ncol=5, nrow=7)
    identicalRankScore <- craneScoreFunction(identicalRank)
    ...
    smallKnownScore <- 1.00607616
    expect_equal(craneScoreFunction(smallTestMat), smallKnownScore)
...
test_that("Sample Score Works", {
    ...
    rankMat <- craneRankFunction(testMat)
    sampleScore <- craneSampleScore(testMat)
    ...
    smallSampleScores <- craneSampleScore(smallTestMat)
    ...
    unorderedSampleScores <- craneSampleScore(
...
test_that("Bootstrap Score Works", {
    ...
    actualRes <- craneBootstrapScore(expression=testExpression,
    ...
    randomRes <- craneBootstrapScore(expression=testExpression,
                                     geneNetwork = orderedGenes,
    ...
    actualRes <- craneBootstrapScore(expression=testExpression,
                                     ...
                                     replace=FALSE,
                                     BPPARAM=param)
    expect_no_error(craneBootstrapScore(expression=testExpression,
                                        geneNetwork = orderedGenes,
                                        phenotype1 = orderedSamples,
                                        phenotype2 = unorderedSamples,
                                        bootstrapIterations = 100,
...
test_that("Compare Phenotypes Works", {
    ...
    resActual <- craneComparePhenotypes(expression = testExpression,
    ...
    unordered1 <- sample(unorderedSamples, 5)
    unordered2 <- sample(unorderedSamples, 5)
    resUnordered <- craneComparePhenotypes(expression = testExpression,
                                        geneNetworkList = geneNetworkList,
                                        phenotype1 = unordered1,
                                        phenotype2 = unordered2,
                                        bootstrapIterations = 100,
    ...
    resList <- craneComparePhenotypes(expression = testExpression,
                                        geneNetworkList = geneNetworkList,
                                        phenotype1 = orderedSamples,
                                        phenotype2 = unorderedSamples,
                                        bootstrapIterations = 100,
