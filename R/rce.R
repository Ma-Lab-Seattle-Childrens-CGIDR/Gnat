...
# SHOULD BE REWRITTEN TO USE THE PAIRWISE ONLY METHOD, THIS MAY STILL HAVE ISSUES WITH TIES
...
rce <- function(gene_expression, gene_index_list, phenotype1, phenotype2,
                iterations=1000, parallel=TRUE, cores=4, replace=TRUE,
                seed1=NULL, seed2=NULL, as.frame=TRUE){
  pvals <- lapply(gene_index_list, rce.compare_phenotypes,
...
#' @param summary.function Function, takes a vector of floats and returns a
#'    float. Will be passed the distribution generated by bootstrapping and
#'    result will
...
#'
#' @return List of the p-value, and the summary statistic for each of the
#'    phenotype distributions. The p-value comes fom the two sample
#'    Kolmogorov-Smirnov test, where the null and alternative hypotheses are:
...
rce.compare_phenotypes <- function(gene_index, gene_expression, phenotype1,
                                   phenotype2, summary.function=stats::median,
                                   summary.name="median",
                                   phenotype.names=NULL,
                                   bootstrap_iterations=1000, parallel=TRUE,
                                   cores=4, replace=TRUE, seed1=NULL,
                                   seed2=NULL){
  ...
  if(is.null(phenotype.names)){
    phenotype.names <- c("p1", "p2")
  }
  summary.stat.p1 <- summary.function(p1.dist)
  summary.stat.p2 <- summary.function(p2.dist)
  stat.names <- paste(phenotype.names, ".", summary.name, sep="")
  pval <- stats::ks.test(p1.dist, p2.dist)$p.value
  names <- c("p.value", stat.names)
  result <- list(pval, summary.stat.p1, summary.stat.p2)
  names(result) <- names
  result
}
...
# This should all be converted to pairwise, that
#   may help alleviate ties
